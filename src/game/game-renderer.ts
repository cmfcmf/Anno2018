...
export default class GameRenderer {
    ...
    private moveCameraToStartPosition(myPlayerId: number) {
        const myKontor = this.world.kontors.find((kontor) => kontor.playerId === myPlayerId);
        if (myKontor !== undefined) {
            const position = myKontor.position;
            const kontorIsland = this.world.islands.find((island) => island.id === myKontor.islandId);
            this.moveTo(new PIXI.Point(
