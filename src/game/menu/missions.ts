...
export default class Missions implements ScreenConfig {
    public buttons = [
        ...
        async (stage: PIXI.Container) => {
            this.currentPage = Math.min(Math.ceil(this.newGameLines.length / this.ROWS) - 1, this.currentPage + 1);
            await this.renderNewGame(stage);
        },
    ];
    ...
    private newGameLines: Array<{name: string, difficulty?: number, file?: WebKitFileEntry}> = [];

    private currentPage = 0;
    private gamParser: GAMParser;

    constructor(private readonly fs: FileSystem, private readonly menuStructure: MenuStructure) {
    ...
    public async renderNewGame(stage: PIXI.Container) {
        ...
        for (let i = 0; i < this.ROWS; i++) {
            ...
            sprite.visible = i + currentTopItem < this.newGameLines.length &&
    ...
    public async renderLoadGame(stage: PIXI.Container) {
        const saves = (await this.fs.ls("/saves", ".gam"))
            .filter((save) => save.name !== "lastgame.gam");
    ...
    private async getNewGameLines() {
        ...
        missions.filter((mission) => mission.missionNum !== -1).forEach((mission) => {
            this.newGameLines.push({name: mission.name, file: mission.file as WebKitFileEntry});
        });
        ...
        customMissions.forEach((mission) => {
            this.newGameLines.push({name: mission.name.replace(".szs", ""), file: mission as WebKitFileEntry});
        });
    }

    private async loadMissions() {
        const missions = await Promise.all(
            (await this.fs.ls("/missions-original", ".szs"))
                .map(async (mission) => {
                    ...
                    return {blocks, ranking, campaignNum, missionNum, name: mission.name, file: mission};
                }),
        );
        missions.sort((a, b) => {
