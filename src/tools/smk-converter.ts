...
if (args.length !== 4) {
  console.error(
    "Please provide the input path to a .smk file as well as the output path to a .mp4 file."
  );
  process.exit(1);
}
...
(async () => {
  ...
  const mp4Data: Uint8Array = await smkParser.parse(
    fs.readFileSync(inFilePath)
  );

  fs.writeFileSync(outFilePath, mp4Data);

  console.log(
    `Original file size: ${fileSize(fs.statSync(inFilePath).size)} MB`
  );
